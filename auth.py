...
def mostrar_asistencia(hoja_asistencia, hoja_colaboradores, registro_mod=None, razon=None):
    ...
    # AUTO-SYNC ESTADO: una sola vez por sesión (no en cada render)
    # Actualiza ESTADO en asistencia con el estado real de colaboradores
    if not st.session_state.get("asis_estado_sincronizado"):
        try:
            df_colab_sync = leer_colaboradores_drive(hoja_colaboradores)
            if not df_colab_sync.empty and "DNI" in df_colab_sync.columns and "ESTADO" in df_colab_sync.columns:
                df_colab_sync["DNI"] = df_colab_sync["DNI"].apply(normalizar_dni)
                estado_map = df_colab_sync.drop_duplicates("DNI").set_index("DNI")["ESTADO"].to_dict()
                df_total["ESTADO"] = df_total["DNI"].apply(
                    lambda d: estado_map.get(normalizar_dni(str(d)), "ACTIVO")
                ).str.strip().str.upper()
                del df_colab_sync, estado_map  # liberar RAM inmediatamente
            st.session_state["asis_estado_sincronizado"] = True
        except Exception:
            st.session_state["asis_estado_sincronizado"] = True  # no bloquear si falla
    ...
    if df_editor_base.empty:
        ...
        # Construir editor: TODO string para evitar React #185
        ...
        # CRÍTICO: convertir TODO a string incluyendo ROW_SHEET
        # Tipos mixtos (int + str) causan React error #185
        df_editor = df_editor_build[columnas_editor].fillna("").astype(str)
        df_editor = df_editor.replace({"nan": "", "None": "", "none": "", "0": ""})
        df_editor["ROW_SHEET"] = df_editor_build["ROW_SHEET"].astype(str)
        df_editor[col_hoy] = df_editor[col_hoy].apply(limpiar_marca)

        disabled_cols = [c for c in df_editor.columns if c != col_hoy]
        column_config = {
            "ROW_SHEET": st.column_config.TextColumn("FILA", width="small", disabled=True),
            col_hoy: st.column_config.SelectboxColumn(
                col_hoy, options=MARCAS_PRESENCIALIDAD, width="small"
            ),
        }
        ...
        editado = st.data_editor(
            df_editor,
            use_container_width=True,
            height=min(460, 50 + len(df_editor) * 35),
            hide_index=True,
        ...
        # Detectar A-BM sin sustento (se muestra ANTES del botón guardar)
        faltantes_sustento = detectar_abm_sin_sustento(
            pd.DataFrame(editado).fillna(""), df_original, col_hoy
        )
        if faltantes_sustento:
            primero = faltantes_sustento[0]
            st.warning(
                f"⚠️ {len(faltantes_sustento)} A-BM sin sustento — "
                f"**{primero['nombre']}** (DNI {primero['dni']}). "
                "Adjunta el documento antes de guardar."
            )
        ...
        guardar_pres = st.button(
            "💾 Guardar Presencialidad",
            key="btn_guardar_presencialidad",
            use_container_width=True,
        )
            ...
            with st.spinner("⏳ Guardando en Google Drive..."):
                try:
                    df_editado = normalizar_para_guardado(
                        pd.DataFrame(editado).fillna(""), col_hoy
                    )
                    if df_editado.empty or "ROW_SHEET" not in df_editado.columns:
                        st.warning("⚠️ No se pudo leer el editor. Recarga la página.")
                    else:
                        faltantes = detectar_abm_sin_sustento(
                            df_editado, df_original, col_hoy
                        )
                        if faltantes:
                            st.error(
                                f"❌ {len(faltantes)} A-BM sin sustento. "
                                "Adjunta el archivo antes de guardar."
                            )
                            st.stop()

                        sustentos_guardados = guardar_sustentos_bm_en_historico(
                            df_editado, col_hoy
                        )
                        ...
                        if not updates:
                            st.info("ℹ️ Sin cambios nuevos para guardar.")
                        else:
                            for i in range(0, len(updates), 100):
                            ...
                            st.session_state["asis_guardado_msg"] = (
                                ...
                                f"**{len(updates)} registros** actualizados"
                                + (f" | **{sustentos_guardados} sustentos A-BM** guardados en Drive" if sustentos_guardados else "")
                            )
                            st.rerun()
                except Exception as e:
